...
export type LogType = 'system' | 'error' | 'logs_started' | 'logs_stopped' | 'log_line' | 'log_error';
  ...
  const bufferMaxSize = 1000;
    ...
    addLogEntry({
      ...
      message: `Connection lost. Reconnecting in ${Math.round(
        delay / 1000
      )} seconds... (Attempt ${reconnectAttemptsRef.current}/${
    ...
    const highlightSearchTerms = useCallback((message: string, filterText: string): string => {
